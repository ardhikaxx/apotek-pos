            ...
            `<div class="col-md-6 col-xl-4">
                <div class="search-result-item p-3 h-100 d-flex flex-column" onclick="addToCart(${p.id},'${p.name.replace(/'/g,"\\'")}',${p.selling_price},${p.stock},'${p.unit}')">
                    <div class="d-flex justify-content-between mb-2">
                        <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill small">${p.category.name}</span>
...
document.getElementById('btn-checkout').addEventListener('click', function () {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.getElementById('btn-print').href = `/admin/transactions/${data.transaction_id}/pdf`;
